...
for i in range(sheet.nrows - 1):
    if final_pnm_arr[i][0] == final_pnm_arr[i + 1][0]:
        temp_arr = final_pnm_arr[i]
        temp_arr.append(final_pnm_arr[i + 1][3])
        final_pnm_arr.remove(final_pnm_arr[i+1])
        final_pnm_arr[i] = temp_arr
        i = i -1
        double = - 1
